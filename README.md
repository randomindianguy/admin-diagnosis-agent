...
The numbers: 40/40 basics, 39/40 nuance — and, under adversarial, multi-turn, and scope-refusal pressure, 8/10, 6/8, 5/5. Every robustness miss but one errs toward refusing rather than toward a wrong grant, which is the thesis holding under stress, not failing it.
...
Five tiers, scored independently — the first two measure correctness, the last three stress the refuse-first thesis under pressure:
...
- **Adversarial** (10 cases) — authority claims, urgency, social proof, prompt injection. **8 / 10.**
- **Multi-turn** (8 cases) — push-back, refuse-then-clarify, context evolution. **6 / 8.**
- **Scope refusal** (5 cases) — bulk provisioning, admin tasks, off-topic. **5 / 5.**

Under adversarial and multi-turn pressure the system never wrongly *grants* access — three of the four robustness misses are over-refusals, the system erring conservative as the thesis intends. The one over-permissive miss is a known architectural gap: a request mixing a resource you have with one you don't is resolved on the half it can satisfy. Both gaps are filed, not patched — the eval measures where the thesis holds, not a polished number.

Methodology: seed-and-mutate. Golden cases authored manually, then mutated along seven failure axes; the robustness tiers add adversarial, multi-turn, and scope probes. Graders validated against deliberately bad outputs before being trusted to discriminate good ones.

Pipeline in `eval/`. Run with `npm run validate` (correctness) and `npm run validate:robustness` (robustness tiers).
